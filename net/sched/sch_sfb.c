...
static void increment_one_qlen(struct sk_buff *skb, int filter,
	struct sfb_sched_data *q)
{
	int i;
...
static void increment_qlen(struct sk_buff *skb, struct sfb_sched_data *q)
{
...
static void decrement_one_qlen(struct sk_buff *skb, int filter,
	struct sfb_sched_data *q)
{
	int i;
	for (i = 0; i < q->numhashes; i++) {
...
static inline void decrement_prob(int filter, int bucket, unsigned hash,
	struct sfb_sched_data *q)
{
...
static inline void increment_prob(int filter, int bucket, unsigned hash,
	struct sfb_sched_data *q)
{
	q->buckets[filter][bucket][hash].pm =
...
static int sfb_rate_limit(struct sk_buff *skb, struct sfb_sched_data* q)
{
